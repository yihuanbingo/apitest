...
if ( isset($_GET['do']) )
{
	...
	else if($_GET['do'] == 'setname')
	{
	  if( isset($_POST['nickname']) )
	  {
	      $return = array("status"=>"error","response"=>"");
		  ...
		  {    
		      ...
		      if( PHPSay::getMemberCount($DB,"nickname",$nickname) != 0 )
			   {
				   $return['response'] = "昵称已被占用，换一个吧 ^_^";
			   }
			   else
			   {
			       $data = array(
	                      'nickname'=>$nickname,
	                      'headimgurl'=>substr($_SESSION['tmpUser']['headimgurl'],0,-2),   //去掉最后两个尺寸参数字符
	                      'regtime'=>time(),
					      'openid'=>$_SESSION['tmpUser']['openid'],
					      );
		           $DB->query( $DB->insert_sql("`phpsay_member`",$data) );
		           $sql = "select uid, nickname, groupid, headimgurl from phpsay_member where openid='".$_SESSION['tmpUser']['openid']."' ";
	...
	else if ( $_GET['do'] == "SignUp" )
	{
		if( isset($_POST['email'],$_POST['nickname'],$_POST['password']) )
		{
			...
			$nickname = filterCode($_POST['nickname']);
	...
	else if ( $_GET['do'] == "resetPassword" )
	{
		if( isset($_GET['code'],$_POST['password']) )
		{
			...
			$codeArr = PHPSay::getResetPasswordCode($DB,"code",filterCode($_GET['code']));
	...
	else if ( $_GET['do'] == "password" )
	{
		...
		if( isset($_GET['code']) )
		{
			if( $_GET['code'] != "" )
			{
				...
				$codeArray = PHPSay::getResetPasswordCode($DB,"code",filterCode($_GET['code']));
