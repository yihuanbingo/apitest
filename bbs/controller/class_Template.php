<?php
	class Template
	{
		...
		function output ( $_top = '' )
		{
			...
			if (strlen($cpl_file_name) > 0)
			{
				...
				if ($compile_template)
				{
					$this->parser = new TemplateParser($this->template_dir . $this->tpl_file);
		...
		function use_cache ( $key = '' )
		{
			if (empty($_POST))
			{
				$this->cache_filename  =  $this->cache_dir . 'cache_'.$key.'.html';
		...
		{
			file_put_contents($this->cache_filename,$output);
			return $output;
		}
	...
	class TemplateParser
	{
		...
		function TemplateParser ( $template_filename )
		{
		...
		function compile( $compiled_template_filename = '' )
		{
			...
			if(preg_match("/<!-- INCLUDE/i", $this->template))
			{
				while ($this->count_subtemplates() > 0)
				{
					...
					{
						if(is_file($this->template_dir . "/$subfile"))
			...
			if (preg_match_all('/{:([a-zA-Z0-9_]+):([^}]*)}/', $page, $var))
			{
				foreach ($var[2] as $cnt => $tag)
				{
					...
					{
						$header  .=  "include \"".dirname(__FILE__)."/../extensions/".$extension.".php\";\n";
					...
					elseif (strpos($tag, ','))
					{
						...
						if (preg_match('/^([a-zA-Z_]+)/', $addparam, $match))
						{
							...
							$code  =  "<?php\n$cmd _$extension(\$$block"."['$skalar'],\$$nextblock"."['$nextskalar']"."$addparam);\n?>\n";
						}
						else
						{
							$code  =  "<?php\n$cmd _$extension(\$$block"."['$skalar'],$addparam);\n?>\n";
						}
					}
					else
					{
			...
			{
		        if ( file_put_contents($compiled_template_filename, $page) )
		        {
			        return true;
			    }
			    else
